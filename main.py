class DynamicArray:
    ...
    def add(self, index, value):
        save = value
        for x in range(index, len(self)):
            if x == index:
                save, self[index] = self[index], value
            else:
                save, self[x] = self[x], save
        return self + [save]
    ...
    def remove_at(self, position):
        ...
        return [self[i] for i in range(l) if i != position]
